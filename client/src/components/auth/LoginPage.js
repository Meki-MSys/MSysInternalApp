  ...
  const login = useGoogleLogin({
    onSuccess: async (response) => {
      try {
        ...
        if (res.data) {
          const { email, sub, email_verified, name, picture } = res.data;
          LoginAction({ email, sub, email_verified, name, picture });
          setTimeout(() => {
            navigate("/dashboard");
          }, 100);
        }
  ...
  return (
    <div className="auth-container">
      <AuthLayout>
        ...
        <div className="">
          <button className="google-button my-3" onClick={login}>
